...
public class BookingNotificationWorker {
    ...
    private final BookingNotificationProcessor processor;
    private final AppProperties properties;
    private final ExecutorService executor;
    private final Semaphore permits;

    public BookingNotificationWorker(
            BookingEventRepository repository,
            BookingNotificationProcessor processor,
    ...
    public void runOnce() {
        ...
        for (String eventId :
                repository.findReadyEventIds(now, properties.getLineWorkerConcurrency())) {
            ...
            try {
                ...
                permits.release();
                int attempts = repository.findEvent(eventId)
                        .map(BookingEventRepository.BookingEventRow::attempts)
                        .orElse(1);
                repository.markRetryOrFailed(
                        eventId,
                        attempts,
                        "Booking notification executor rejected the task",
                        Instant.now());
            }
        }
    }
