...
- 加購只屬於單一主服務。V9 Java 遷移拆分舊共用加購並新增 `add_on_id` 唯一限制，
  保留 V1～V8 原檔及既有預約快照。不同主服務可有同名且不同價格的加購。
- 新增與編輯加購使用主服務範圍端點，驗證租戶及主服務歸屬，並鎖定主服務列，
  在同一交易新增與綁定。舊全域更新端點僅可修改未綁定項目；已綁定項目回傳 409。
  舊未綁定加購保留於資料庫，不自動猜測其適用服務；商家可在主服務內重新建立。
