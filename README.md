...
> 面向中小型商家的多租戶 LINE 客服平台，整合 AI 知識庫問答、人工客服案件、店家人員管理與即時通知；線上預約為選用功能。
...
新建店家預設「純客服」，適用零售、餐飲及其他不需要線上預約的店家。註冊時可勾選「啟用線上預約」，既有店家也能在「商家設定 → 店家功能」切換。

- **純客服**：知識庫、回答測試、人員綁定、轉人工與案件結案；隱藏預約服務設定和快捷入口。
- **客服＋預約**：增加預約服務／加購、時段查詢、建立與管理預約。
- **停用預約**：立即停止接受新預約，包括舊連結與 API；保留歷史資料、查詢、取消及通知。客服總覽保留「既有預約管理」，LINE 可輸入「查詢預約」、「取消預約」或人員的「店家管理」。
- 商品介紹、營業時間、地址及常見問題請整理至知識庫並發布，兩種模式都能使用。
